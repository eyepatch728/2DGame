...
//public class CanadianAnimalsGameManager : MonoBehaviour
//{
//    public static CanadianAnimalsGameManager Instance;
...
//    [HideInInspector]public List<GameObject> animalsWithPopupShown = new List<GameObject>();  // Track animals whose popup has been shown
...
//    {
//        gameActive = false;
//        CanadianAnimalsPopups.instance.ShowEndGamePopup();
//    }
...
//    }
//    public IEnumerator ShowAnimalInfo(GameObject animal)
//    {
...
//            // Stop the audio after the popup time has passed
...
public class CanadianAnimalsGameManager : MonoBehaviour
{
    ...
    [SerializeField] private TMP_Text animalDescription;
    [SerializeField] private TMP_Text visitedAnimalsText;
    [HideInInspector] public List<GameObject> animalsWithPopupShown = new List<GameObject>();
   ...
   void Update()
{
    ...
    if (isGameStart())
    {
        foreach (GameObject animal in animals)
        {
            // Ensure the animal is within range and hasn't been visited yet
            if (Vector2.Distance(player.transform.position, animal.transform.position) < 4f && !isPopupActive && !animalsWithPopupShown.Contains(animal))
            {
                SoundManager.instance.PlaySingleSound(0);
                StartCoroutine(ShowAnimalInfo(animal));
    ...
    public IEnumerator ShowAnimalInfo(GameObject animal)
    {
        if (animalsWithPopupShown.Contains(animal))
        {
            yield break; // Exit early if the animal has already been visited
        }
    ...
    private void CloseAnimalPopup()
    {
        ...
        if (currentAnimal != null && !animalsWithPopupShown.Contains(currentAnimal))
        {
            animalsWithPopupShown.Add(currentAnimal);
            animalsVisited++; // Only increment counter when adding a new animal
        }
    }
