...
public class ColorGameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (panelOnComplete.activeSelf && Input.GetMouseButtonDown(0))
        {
            ...
            {
                // Close panel and proceed immediately on user click
    ...
    {
        gameActive = false;
        MexicColoringPopups.instance.ShowEndGamePopup();
    }
    ...
    private void SetUpLevel()
    {
        ...
        ColorFiller colorFiller = FindObjectOfType<ColorFiller>();
        if (colorFiller != null)
        {
    ...
    {
        float reducedWidth = sprite.rect.width * scaleFactor;
        float reducedHeight = sprite.rect.height * scaleFactor;
    ...
    private IEnumerator PlayDescriptionWithAudio()
    {
        ...
        {
            panelOnComplete.SetActive(false);
            StartNewLevel(false); // Use delayed transition for automatic progression
        }
    }
