...
public class EiffelTowerCubeController : MonoBehaviour
{
    ...
    void OnMouseDrag()
    {
        if (EiffelTowerManager.Instance.isGameStart())
        {
            ...
            if (!placedCorrectly)
            {
                if (Vector2.Distance(transform.position, CircuitSlot.position) < 0.5f)
                {
                    if (CorrectCube)
                    {
                        ...
                        EiffelTowerCircuitManager.Instance.CorrectPlacement();
                        //EiffelTowerManager.Instance.Win();
                    }
                    else
                    {
