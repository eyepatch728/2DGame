...
public class AntarcticaColoringPopups : MonoBehaviour
{
    public static AntarcticaColoringPopups instance;
    ...
    private bool isTutorialShown = false;  // New flag to track if tutorial has been shown
    ...
    void Update()
    {
        if (!popupPanel.activeSelf || tutorialPanel.activeSelf) return;  // Don't process input if popup is inactive or tutorial is showing
    ...
    {
        isTutorialShown = true;  // Mark tutorial as shown
    ...
    IEnumerator CloseTutorialAfterDelay(float delay)
    {
        ...
        if (tutorialPanel.activeSelf)  // Only close if still active
        {
            tutorialPanel.SetActive(false);
            AntarcticaAnimalPaintingManager.Instance.StartGame();
        }
    }
